...
**AgentWatch 是一个 MCP 中间件产品，部署在用户的 AI Agent 和 OKX MCP Server 之间，拦截并分析每一次工具调用，通过行为基线建模检测异常操作，为 Agent 提供运行时安全监测与风险告警。**
...
│
├── 本地层：MCP 中间件（开源/免费）
│   ├── 部署位置：用户本地（Claude Code / Cursor / CLI 环境）
│   ├── 核心功能：工具调用拦截 + 参数捕获 + 实时规则检测 + 本地日志
│   ├── 数据流向：
│   │   Agent → AgentWatch MCP Proxy → OKX MCP Server
│   │             ↓ 拦截记录
│   │         本地日志文件 (~/.agentwatch/log.jsonl)
│   │             ↓ 用户授权上传
│   │         AgentWatch 云端分析服务
│   └── 开源价值：建立信任、社区贡献、引流获客
│
└── 云端层：ASP 分析服务（付费）
    ├── 部署位置：AgentWatch 云端服务器
    ├── 核心功能：行为基线学习 + 深度异常检测 + 审计报告 + 历史分析
    ├── 收费模式：通过 OKX Payment SDK，按 API 调用次数付费
    └── 接入方式：本地中间件自动将数据上传到云端服务
```
...
- **主要用户**：使用 OKX AI Agent 进行自主交易/操作的个人用户
...
│                                                                                  │
│  ┌─────────────────┐     ┌─────────────────┐     ┌───────────────────────────┐  │
│  │   AI Agent      │◄────┤  MCP Client     │◄────┤   AgentWatch Local        │  │
│  │  (Claude/Cursor)│     │  (stdio)        │     │   (MCP Proxy Middleware)  │  │
│  └─────────────────┘     └─────────────────┘     └────────────┬──────────────┘  │
│                                                               │                  │
└───────────────────────────────────────────────────────────────┼──────────────────┘
                                                                │
                       ┌────────────────────────────────────────┼──────────────────┐
                       │           AgentWatch Local Layer        │                  │
                       │                                        ▼                  │
                       │  ┌──────────────────────────────────────────────────┐     │
                       │  │         MCP Proxy Core (stdio转发)               │     │
                       │  │  • JSON-RPC 请求/响应拦截                        │     │
                       │  │  • 工具调用参数提取                              │     │
                       │  │  • 双向管道管理                                  │     │
                       │  │  [V0] 延迟目标: <0.1ms 纯转发 / <5ms 带分析      │     │
                       │  └────────────────────┬─────────────────────────────┘     │
                       │                       │                                    │
                       │         ┌─────────────┼─────────────┐                     │
                       │         ▼             ▼             ▼                     │
                       ...
                       ▼                  ▼                  ▼
              ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
              │  OKX MCP     │   │  Cloud API   │   │  Local Store │
              │  Server      │   │  (WebSocket) │   │  (SQLite)    │
              │  (真实工具)  │   │  (上报/同步) │   │  (基线/日志) │
              └──────────────┘   └──────┬───────┘   └──────────────┘
                                       │
═══════════════════════════════════════╪══════════════════════════════════════════════
                       ...
                       ┌───────────────┼────────────────────────────────┐
                       │               ▼                                │
                       │        AgentWatch Cloud Layer (ASP)            │
                       │                                               │
                       │  ┌──────────────┐  ┌──────────────────────┐  │
                       │  │  API Gateway │  │  Real-time Stream    │  │
                       │  │  (Auth/Rate) │  │  Processing (Kafka)  │  │
                       │  └──────┬───────┘  └──────────┬───────────┘  │
                       │         │                      │              │
                       │  ┌──────┴──────────────────────┴──────┐      │
                       │  ▼                                      ▼      │
...
**设计决策**：选择 `stdio` 而非 `HTTP` 代理——OKX MCP Server 是 stdio 类型，stdio 代理完全透明无需改配置。

```
┌──────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ AI Agent │────▶│ MCP Client   │────▶│ MCP Proxy    │────▶│ L0 规则引擎  │
│          │     │ (stdio out)  │     │ (拦截解析)    │     │ (<10ms 匹配) │
└──────────┘     └──────────────┘     └──────────────┘     └──────┬───────┘
                                                                    │
                                                                    ▼
┌──────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ AI Agent │◄────│ MCP Client   │◄────│ 响应注入     │◄────│ 决策路由器    │
│          │     │ (stdio in)   │     │ (标记/告警)   │     │ (综合评分)    │
└──────────┘     └──────────────┘     └──────────────┘     └──────┬───────┘
                              ▲                                    │
                              │                                    │
...
│                                                  ▼               ▼               ▼
│  ┌──────────────┐  ┌──────────────┐         ┌────────┐    ┌──────────┐    ┌──────────┐
│  │ 决策结果     │  │ 响应处理     │         │ ALLOW  │    │ BLOCK    │    │ CHALLENGE│
│  │ • 原始响应   │  │ • 注入标记   │         │ (放行) │    │ (拦截)   │    │ [V2]     │
│  │ • 安全提示   │  │ • 告警信息   │         └────┬───┘    └────┬─────┘    └──────────┘
│  └──────────────┘  └──────────────┘              │             │
│                                                  │             │
...
[MCP Client Request]
    ↓ (stdio stdin)
[MCP Proxy Core] 解析 JSON-RPC (<0.1ms)
    ↓
[Parameter Extractor] 提取 tool_name + params (<0.1ms)
    ↓ (并行)
    ├─▶ [L0 规则引擎] 快速匹配 (<10ms)
    │       ↓
    │   [规则匹配结果] allow/block
    ...
    └─▶ [决策路由器] 综合判定 (<0.1ms)
            ↓
        [三种决策]
            ├─▶ ALLOW  → 转发到 OKX MCP Server → 返回原始响应
            ├─▶ BLOCK  → 返回错误响应 (不转发)
            └─▶ CHALLENGE [V2] → 注入人机验证
```
...
| L1 统计检测 | 50ms | ✅ | Welford + CUSUM + EWMA |
| 决策路由 | 0.1ms | ✅ | 简单 if-else |
...
AgentWatch 检测引擎采用 **四级分层架构**，每一层具有不同的延迟预算、检测能力和资源消耗特征。事件从 L0 逐层向上流动，每一层可以独立触发告警，也可以将上下文传递给更高层进行深度分析。
...
│       ▼                                                                     │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ L0 规则引擎 (<10ms)          本地内存    内存<5MB                  │    │
│  │  · 关键词/正则匹配  · 硬阈值  · 黑白名单  · 紧急拦截               │    │
│  └─────────────────────────┬───────────────────────────────────────────┘    │
│                            │ 可疑? 升级到 L1                                │
│       ▼ 正常               ▼ 可疑                                          │
│  [放行]               ┌──────────────────────────────────────────────┐      │
│                       │ L1 统计引擎 (<50ms)    本地内存    内存<15MB │      │
│                       │  · Z-score  · CUSUM  · EWMA  · Markov链     │      │
│                       │  · 滑动窗口频率  · 时序异常                   │      │
│                       └──────────────┬───────────────────────────────┘      │
│                                      │ 异常? 升级到 L2                      │
│                         正常 ▼       ▼ 异常                                │
│  [放行]            ┌──────────────────────────────────────────────┐        │
│                    │ L2 轻量ML (<50ms)    本地内存    内存<25MB   │        │
│                    │  · 孤立森林  · 工具共现矩阵  · 轻量聚类      │        │
│                    └──────────────┬───────────────────────────────┘        │
│                                   │ 深度分析? 升级到 L3 (异步)              │
│                      正常 ▼       ▼ 深度分析                               │
...
│  ┌──────────────────────────────────────────────┐   │
│  │           Rule Index (多维索引)                │   │
│  │  Trie树(前缀)  │  Aho-Corasick(多模式)        │   │
│  │  Hash表(精确)  │  RegexCache(正则缓存)         │   │
│  └──────────────────────┬───────────────────────┘   │
│                         ▼                            │
│  ┌──────────────────────────────────────────────┐   │
│  │           Match Orchestrator                 │   │
...
│  ┌──────────────────────────────────────────────┐   │
│  │          Action Executor                     │   │
│  │  BLOCK │ WARN │ ESCALATE │ LOG │ BYPASS      │   │
│  └──────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────┘
```
...
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌─────────┐ │
│  │ Welford  │ │  CUSUM   │ │  EWMA    │ │ Sliding │ │
│  │ Z-score  │ │ 累积和   │ │指数移动均│ │ Window  │ │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬────┘ │
│       └─────────────┼────────────────┘              │
│                     ▼                                │
│  Sequence Detection                                  │
...
│  │   分布          │    │   统计          │               │
│  │                 │    │                 │               │
│  │ okx_mcp::get_   │    │ place_order.sz  │               │
│  │ account_balance │    │   mean=0.5      │               │
│  │   33.7%         │    │   std=2.3       │               │
│  │ okx_mcp::get_   │    │   p95=3.0       │               │
│  │ ticker 26.6%    │    │   p99=10.0      │               │
│  │ okx_mcp::place_ │    │                 │               │
│  │ order 13.6%     │    │ transfer.amount │               │
│  │      ...        │    │   mean=500      │               │
│  └─────────────────┘    │   median=100    │               │
│                         └─────────────────┘               │
│  ┌─────────────────┐    ┌─────────────────┐               │
...
│  │          ticker │    │   8:  0.08      │               │
│  │ get_       0.45 │    │   9:  0.12      │               │
│  │ balance ->      │    │  10:  0.15 (peak)│              │
│  │ place_   0.30   │    │  11:  0.12      │               │
│  │ order           │    │  12:  0.10      │               │
│  │ history  0.15   │    │  14:  0.08      │               │
│  │                 │    │   2:  0.005(rare)│              │
│  └─────────────────┘    └─────────────────┘               │
│  ┌─────────────────┐    ┌─────────────────┐               │
│  │ 5. 资产偏好      │    │ 6. 交互Agent    │               │
...
│  │  ETH-USDT  30%  │    │       │         │               │
│  │  PEPE-USDT  5%  │    │       ▼         │               │
│  │  SOL-USDT  15%  │    │  [User Agent] ──► [OKX MCP]     │
│  │  DOGE-USDT  5%  │    │       │         │               │
│  │                 │    │       ▼         │               │
│  │  new asset      │    │    [Agent B]    │               │
│  │  -> flag        │    │    [rare]       │               │
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Level 1: 无数据期 (0-10 次调用)                            │
│  ┌──────────────────────────────────────┐                  │
│  │ • 仅使用 L0 规则引擎 (硬编码阈值)     │                  │
│  │ • 行业通用安全规则兜底                │                  │
│  │ • 所有参数异常标记为 "new_behavior"   │                  │
│  │ • 不触发告警，仅记录                  │                  │
│  └──────────────────────────────────────┘                  │
│                          │                                  │
│                          ▼ (累积 10+ 次调用)                │
│  Level 2: 初建期 (10-100 次调用)                            │
│  ┌──────────────────────────────────────┐                  │
│  │ • 启用 L1 统计引擎 (自适应阈值)       │                  │
│  │ • Welford 在线学习启动               │                  │
│  │ • Z-score 阈值放宽到 5-sigma         │                  │
│  │ • 保守告警策略 (仅 WARN)              │                  │
│  └──────────────────────────────────────┘                  │
│                          │                                  │
│                          ▼ (累积 100+ 次调用 / 24h)         │
...
npx @agentwatch/mcp-proxy --config ~/.agentwatch/config.yaml
...
V0:                          V1:
┌──────────────┐            ┌──────────────┬──────────────┐
│ MCP Proxy    │            │ MCP Proxy    │ Cloud API    │
│ ├─ L0 Rules  │     →      │ ├─ L0 Rules  │ ├─ Gateway   │
│ ├─ L1 Simple │            │ ├─ L1 Full   │ ├─ WS Server │
│ ├─ Logger    │            │ ├─ Baseline  │ ├─ Analysis  │
│ └─ Config    │            │ ├─ Logger    │ ├─ Reports   │
│              │            │ └─ Config    │ └─ Alerts    │
│ 本地-only     │            │              │              │
│              │            │ 本地 + 云端    │  Dashboard   │
└──────────────┘            └──────────────┴──────────────┘
...
V1:                          V2:
┌──────────────┬──────────┐  ┌──────────────┬──────────┬──────────┐
│ MCP Proxy    │ Cloud    │  │ MCP Proxy    │ Cloud    │ ML GPU   │
│ ├─ L0 Rules  │ ├─ API   │  │ ├─ L0 Rules  │ ├─ API   │ ├─ LSTM  │
│ ├─ L1 Stats  │ ├─ WS    │  │ ├─ L1 Stats  │ ├─ WS    │ ├─ GNN   │
│ ├─ Baseline  │ ├─ Report│  │ ├─ L2 ML     │ ├─ Report│ ├─ BOCPD │
│ └─ Logger    │ └─ Alert │  │ ├─ Baseline  │ ├─ Alert │          │
│              │          │  │ └─ Logger    │ └─ ML Job│          │
│              │Dashboard │  │              │          │          │
└──────────────┴──────────┘  │              │ Dashboard│          │
                             │              │ ├─ Policy│          │
...
## Week1 Day7 源码落地同步（V0 MVP 代码映射）

| 架构章节 | 源码落地 | 状态 |
|----------|----------|------|
| §3 MCP 中间件 | `MCPProxyCore` + `bootstrap` | Week1 ✅ |
| §5 行为基线 | `StatEngine` + `baseline.ts` 静态种子 | 无 SQLite ⏸ |
| §5.1 FIFO 双流 | `bootstrap.ts` GatewayClientInput | ✅ 扩展 |
| §5.2 JSON 修复 | `repairGatewayJsonSyntax` | ✅ 扩展 |
| §6 十大场景 | L0 8 规则 + L1 融合 | 见 README 缺口表 |
| §9 风险融合 | `DecisionRouter` | ✅ |
| 全局常量 | `packages/shared/constants.ts` | Day7 收拢 |

### 十大场景实现缺口（与 README 一致）

A2A 风险未实现；耗时异常 / 基线偏离仅 L1 弱覆盖；Prompt Injection / 供应链依赖静态规则，无 FalsePositiveController 持久化。

**Week1 结论**：核心链路可发包 `0.1.0`，Week2-3 聚焦 SQLite、云端上报、CLI。
